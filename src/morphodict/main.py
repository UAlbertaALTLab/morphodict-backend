...
async def search(word: str):
    ...
    reqString = "{" + reqString + "}"
    resHandler = "curl -X POST localhost:8000/api/search/ -H 'Content-Type: application/json' -d '{}'".format(
        reqString
    )
...
async def get_word(word: str):
    ...
    word = 'curl localhost:8000/api/word/{}/ -H "Accept: application/json"'.format(
        clean
    )
